...
public class PayPalSucess {
	...
	public PayPalResult getPayPal(HttpServletRequest request, PayPalConfig1 payPalConfig1 ) {
		PayPalResult payPalResult = new PayPalResult();	
		String[] temp=null;
		String res="";
		try {
			String transId = request.getParameter("tx")  ;
			String authToken =  payPalConfig1.getAuthToken();
			String query = "cmd=_notify-synch&tx=" + transId + "&at=" + authToken; 
			String url=payPalConfig1.getPosturl();
			URL u=new URL(url);
